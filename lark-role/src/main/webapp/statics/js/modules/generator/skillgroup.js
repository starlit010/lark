...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.skillGroup.skillId == null ? "skillgroup/save" : "skillgroup/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.skillGroup),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(skillId){
			$.get(baseURL + "skillgroup/info/"+skillId, function(r){
                vm.skillGroup = r.skillGroup;
            });
		},
