$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'chaChannelId', index: 'cha_channel_id', width: 80 }, 			
			{ label: '', name: 'channelId', index: 'channel_id', width: 80 }, 			
			{ label: '', name: 'appName', index: 'app_name', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.app.appId == null ? "app/save" : "app/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		getInfo: function(appId){
			$.get(baseURL + "app/info/"+appId, function(r){
                vm.app = r.app;
            });
		},
