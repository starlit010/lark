$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '', name: 'asktype', index: 'asktype', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.knowledgeAsk.askId == null ? "knowledgeask/save" : "knowledgeask/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.knowledgeAsk),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "knowledgeask/delete",
				    contentType: "application/json",
		...
		getInfo: function(askId){
			$.get(baseURL + "knowledgeask/info/"+askId, function(r){
                vm.knowledgeAsk = r.knowledgeAsk;
            });
		},
